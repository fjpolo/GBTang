...
always @(posedge clk) begin
    if (~resetn) begin
        ...
        if (init_x == 8'd255)
            init_y <= init_y + 1;
        if (init_y == 8'd255 && init_x == 8'd255)
            initializing <= 0;
        mem_portA_we <= 1;
        mem_portA_addr <= {init_y, init_x};
        mem_portA_wdata <= 2'd3;          // black / darkest palette color
    end else begin
        if (gameboy_vs) begin
            init_x <= 8'd0;
            init_y <= 8'd0;
            mem_portA_we <= 1'b0;
        end else if (gameboy_hs) begin
            init_x <= 8'd0;
            mem_portA_we <= 1'b0;
        end else if (gameboy_valid) begin
            mem_portA_addr <= {init_y, init_x};
            mem_portA_wdata <= gameboy_pixel;
            mem_portA_we <= 1'b1;
            if (init_x == 8'd159) begin
                init_x <= 8'd0;
                if (init_y == 8'd143)
                    init_y <= 8'd0;
                else
...
always_ff @(posedge clk_pixel) begin
    ...
    if (i_reg_aspect_ratio && cx == 11'd347 || ~i_reg_aspect_ratio && cx == 11'd381)
        active <= (cy < 10'd432);
    if (i_reg_aspect_ratio && cx == 11'd932 || ~i_reg_aspect_ratio && cx == 11'd898 || cy >= 10'd432)
        active <= 1'b0;
